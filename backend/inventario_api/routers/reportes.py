...
def get_dashboard(db: Session = Depends(get_db), user: Usuario = Depends(solo_admin)):
    ...
    valor_inventario = db.query(func.sum(Producto.precio_venta * Producto.stock_actual)).scalar() or 0

    ventas_mes = db.query(func.count(Venta.id_venta), func.sum(Venta.total)).filter(Venta.fecha >= hace_30_dias).first()
    ventas_semana = db.query(func.count(Venta.id_venta), func.sum(Venta.total)).filter(Venta.fecha >= hace_7_dias).first()

    ventas_por_dia_raw = db.query(
        func.date(Venta.fecha).label("dia"),
        func.count(Venta.id_venta).label("cantidad"),
        func.sum(Venta.total).label("total")
    ).filter(Venta.fecha >= hace_14_dias).group_by(func.date(Venta.fecha)).order_by("dia").all()

    ventas_por_dia = [{"dia": str(r.dia), "cantidad": r.cantidad, "total": float(r.total or 0)} for r in ventas_por_dia_raw]
    ...
    top_productos = [{"nombre": r.nombre, "total_vendido": int(r.total_vendido), "ingreso": float(r.ingreso or 0)} for r in top_productos_raw]

    productos_stock_bajo = db.query(Producto).filter(Producto.stock_actual <= Producto.stock_minimo, Producto.activo == True).all()
    stock_bajo_lista = [{"nombre": p.nombre, "stock_actual": p.stock_actual, "stock_minimo": p.stock_minimo, "precio_venta": float(p.precio_venta)} for p in productos_stock_bajo]

    distribucion_stock = db.query(Producto).filter(Producto.activo == True).order_by(desc(Producto.stock_actual)).limit(8).all()
    dist_stock = [{"nombre": p.nombre, "stock": p.stock_actual} for p in distribucion_stock]
...
def get_prediccion(db: Session = Depends(get_db), user: Usuario = Depends(solo_admin)):
    ...
    ventas_recientes = db.query(
        Producto.id_producto, Producto.nombre, Producto.stock_actual,
        Producto.stock_minimo, Producto.precio_compra,
    ...
    ).group_by(Producto.id_producto, Producto.nombre, Producto.stock_actual, Producto.stock_minimo, Producto.precio_compra).all()

    ventas_anteriores = db.query(
        Producto.id_producto, func.sum(DetalleVenta.cantidad).label("vendido_prev")
    ).join(DetalleVenta, Producto.id_producto == DetalleVenta.id_producto
    ).join(Venta, DetalleVenta.id_venta == Venta.id_venta
    ...
    for r in ventas_recientes:
        ...
        tendencia = ((vendido_30d - vendido_prev) / vendido_prev * 100) if vendido_prev > 0 else 0
