...
router = APIRouter(prefix="/telegram", tags=["Telegram MCP"])
...
# ── 2. REVISAR Y ALERTAR STOCK BAJO ──────────────────────────────────────────
...
async def verificar_stock_bajo(
    ...
    # Enviar una alerta por cada producto crítico
    enviados = 0
    for p in productos_bajo:
        ok = await alerta_stock_bajo(p.nombre, p.stock_actual, p.stock_minimo)
        if ok:
            enviados += 1

    return {
        "ok": True,
        "productos_alertados": enviados,
        "productos": [p.nombre for p in productos_bajo],
    }


# ── 3. RESUMEN DIARIO MANUAL ──────────────────────────────────────────────────
@router.post("/resumen-hoy")
async def enviar_resumen_hoy(
    db: Session = Depends(get_db),
    user: Usuario = Depends(solo_admin),
):
    hoy = date.today()
...
@router.post("/mensaje")
async def enviar_mensaje_personalizado(
    payload: dict,
    user: Usuario = Depends(solo_admin),
):
